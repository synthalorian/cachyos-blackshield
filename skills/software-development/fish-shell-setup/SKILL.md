...
The canonical copies are versioned in `cachyos-setup`; `handoff-post-reboot.sh` redeploys them after every reboot.

### Versioning function files

```bash
cp ~/.config/fish/functions/<name>.fish ~/Projects/active/cachyos-setup/configs/fish/functions/
cd ~/Projects/active/cachyos-setup && git add configs/fish/functions/<name>.fish && git commit && git push
```
