# Dead Code Cleanup Workflow for Rust Projects

Systematic approach to eliminating compiler warnings without breaking functionality.

## The Pattern (128 → 0 warnings)

### Step 1: `cargo fix` for the easy wins
```bash
cargo fix --bin "openshark" -p openshark --allow-dirty
```
This auto-removes unused imports, unused mut, etc. Typically fixes ~20-30% of warnings.

### Step 2: Module-level `#[allow(dead_code)]` for future-utility APIs
...
- Unused enum variants → remove or comment out
- Never-constructed structs with no consumers → remove
- Test-only functions not in test modules → remove or move to tests

### Step 5: Fix test-only breakage
After cleanup, run `cargo test` — some "dead" code is actually used in tests:
- Fields read only in `#[cfg(test)]` blocks need `#[allow(dead_code)]`
- Test helper functions need `#[allow(dead_code)]` if not all tests use them
...
Zero warnings is the goal, but don't sacrifice API completeness. If a function is part of a planned feature (MCP, gateway, multi-model), `#[allow(dead_code)]` is correct. Remove only code with no plausible future consumer.
