...
## Hermes vs OpenClaw — Do Not Confuse

These are **different tools** that the user runs side-by-side. The user explicitly corrected conflating them.

| | Hermes | OpenClaw |
|---|---|---|
| **What** | AI agent framework (terminal + gateway) | Autonomous coding agent gateway |
| **Config** | `~/.hermes/config.yaml` | `~/.openclaw/openclaw.json` |
| **Binary** | `hermes` (Python venv) | `openclaw` (Node.js via mise) |
| **Model config** | `model.default` + `model.provider` | `agents.defaults.model.primary` + `models.providers` |
| **CLI** | `hermes config set`, `hermes doctor` | `openclaw config set`, `openclaw config validate` |

**When the user says "my model config is broken" and mentions `openclaw`, `kimi-claw`, or shows OpenClaw-specific errors like `Provider kimi is in cooldown`, they mean OpenClaw, NOT Hermes.** Do not default to Hermes config fixes. Ask which tool if unclear, or check for OpenClaw-specific context (`.openclaw.json`, `openclaw` binary, `kimi-claw` plugin).
...
Instead of parsing `hermes gateway status` (which outputs systemd service status), read `~/.hermes/gateway_state.json` directly for platform-level connection data:
...
**PITFALL:** `hermes gateway status` outputs systemd unit status, not platform connections. If you need to know which platforms are connected and their states, read `gateway_state.json` instead.

**PITFALL: `hermes gateway stop` hangs when called programmatically.** The `stop` command prompts for user confirmation (`y/N`) via stdin. When spawned as a subprocess (e.g. from a Rust backend or Flutter app), it blocks forever waiting for input. Fix: pipe "y\n" to stdin before waiting for output:

```rust
// Rust — pipe confirmation via stdin
use std::io::Write;
...
```dart
// Dart — pipe confirmation via stdin
final process = await Process.start('hermes', ['gateway', 'stop']);
...
**PITFALL: `hermes <model>` fails with "invalid choice" error.** Hermes interprets the second positional argument as a subcommand, not a model name. `hermes 35b` is interpreted as `hermes` (defaults to chat) + `35b` as an unrecognized subcommand. The correct forms are:

```bash
# One-shot query with specific model
hermes -m llama-swap/synthclaw-35b-128k "your prompt"

# Interactive chat with specific model
hermes chat --model llama-swap/synthclaw-35b-128k

# Even shorter
hermes chat -m 35b "your prompt"   # if llama-swap config has alias "35b"
```

Always use `-m MODEL` or `--model MODEL` flags. Never pass a model name as a positional argument.
...
| `~/.hermes/.env`              | API keys and secrets **(protected file — cannot use read_file/write_file/patch; must use `terminal` + `sed`)** |
...
## Building Setup Flows for Hermes (GUIs & Automation)

When building a GUI or automated tool that installs/configures Hermes for any user (like Hermes Wingman), use this universal setup pattern:

### Universal Setup Flow

```
1. DETECT  → 2. INSTALL (if needed)  → 3. AUTO-CONFIGURE  → 4. TEST  → 5. DONE
```

### Step 1: Detect

Return structured info about the system state. NEVER hardcode paths — use `which` + `$HOME`:

```rust
// Detect endpoint should return:
{
...
**Binary discovery order:** `which` → `$HOME/.local/bin/` → `/usr/bin/` → `/usr/local/bin/`. The `which` command works on every POSIX system and avoids hardcoded paths.
...
| Failure | Fix |
|---------|-----|
| `pip3 install fails (externally-managed-environment)` | Retry with `--break-system-packages`, or suggest `pipx`, or venv |
| `pip3 not found` | Fall back to `pip`, detect package manager, give OS-specific instructions |
| `Python not found` | Suggest OS package manager (apt/pacman/brew) |

**PITFALL:** `pip install --break-system-packages` should be a retry, not the first attempt — the flag doesn't exist on all Python versions.

### Step 3: Auto-Configure (the key piece)

Scan the environment and write a complete `config.yaml`. Detect:

**Local services (port scan):**
- llama-swap on `localhost:8080` — if found, auto-discover models via `GET /v1/models`
- Ollama on `localhost:11434`

**Cloud providers (env var scan):**

| Env Var | Provider Name | Base URL |
|---------|---------------|----------|
| `OPENAI_API_KEY` | `openai` | `https://api.openai.com/v1` |
| `ANTHROPIC_API_KEY` | `anthropic` | `https://api.anthropic.com/v1` |
| `GEMINI_API_KEY` / `GOOGLE_API_KEY` | `gemini` | `https://generativelanguage.googleapis.com/v1beta` |
| `GROK_API_KEY` / `XAI_API_KEY` | `xai` | `https://api.x.ai/v1` |
| `MISTRAL_API_KEY` | `mistral` | `https://api.mistral.ai/v1` |
| `DEEPSEEK_API_KEY` | `nous` | `https://api.nousresearch.com/v1` |
| `OPENROUTER_API_KEY` | `openrouter` | `https://openrouter.ai/api/v1` |

**Model selection priority:**
1. Local llama-swap model (first discovered) → used as default + fallback chain
2. Cloud provider with known model (openai/gpt-4o-mini, anthropic/claude-sonnet-4, etc.)
3. Preserve existing config model if already set (merge, don't overwrite)

**Config generation:** Always merge with existing config. Don't overwrite providers the user already configured. Write the full config as valid YAML.

### Step 4: Test

Send a minimal probe request to verify the provider works:
...
Check response for:
- HTTP 200 + valid JSON with `choices[0].message.content`
- HTTP 401 → "Authentication failed"
- HTTP 429 → "Rate limited"
- Empty choices → "Model responded but no content (might be loading)"
...
- Connected platforms
- Quick links to Config, Models, Gateway tabs
...
When writing config from auto-configure, **must merge with existing config** — never rebuild from scratch. Naive YAML string building drops sections like `agent`, `delegation`, `display`, `terminal`, etc., silently destroying the user's setup.

**WRONG (string building — drops all non-provider sections):**
```rust
let mut config_yaml = String::new();
config_yaml.push_str(&format!("model: {}\n", default_model));
// ... builds only model + fallback_providers + providers
// agent section, delegation, display, terminal, etc. are LOST
```

**CORRECT (serde_yaml Value deep-merge):**
```rust
// Parse existing config as YAML Value
...
if let Some(mapping) = config_value.as_mapping_mut() {
    mapping.insert("model".into(), default_model.into());
    mapping.insert("providers".into(), providers_value);
    // All other sections (agent, delegation, display, ...) survive untouched
}
...
**Why this matters:** The existing config may contain `model.provider`, `agent.max_turns`, `delegation.model`, `display.skin`, `terminal.backend`, `security.tirith_enabled`, etc. String-building loses every one of them.

**PITFALL:** Use `api_key_env` (env var reference) rather than `api_key` (inline value) for cloud providers discovered from environment variables. This keeps secrets out of the config file and lets the user update env vars without touching config.

**PITFALL (Rust format strings):** When constructing Authorization headers, `format!("Bearer {}", key)` uses `{}` for interpolation. A common mistake is `format!("Bearer *** key)` (literal asterisks) which sends `"Bearer *** key"` instead of the actual key. This silently breaks every API call — the provider sees an invalid auth header and returns 401.

Full endpoint implementation details (Rust Axum): `references/auto-configure-endpoint-design.md`

### Utility Endpoints: Version, Update, Skills

When building a GUI wrapper around Hermes (like Hermes Wingman), three utility endpoints are useful:

**Version** — `GET /hermes/version`:
```rust
async fn hermes_version() -> Json<serde_json::Value> {
    ...
    match output {
        Ok((stdout, stderr, code)) => {
            ...
            Json(json!({ "success": code == 0, "version": output_str }))
        }
...
**Update** — `POST /hermes/update`:
```rust
async fn hermes_update() -> Json<serde_json::Value> {
    let output = std::process::Command::new("pip3")
        .args(["install", "--upgrade", "hermes-agent"])
        .output()?;
    // Return stdout/stderr + success flag
}
```

**Skills** — `GET /hermes/skills`:

**PITFALL:** `hermes skills list` outputs a box-drawing table, not plain text:
```
┏━━━━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━┳━━━━━━━━━┳━━━━━━━━━┓
...
Parse by splitting on `│` (pipe character) from lines that start with it:
...
async fn hermes_skills() -> Json<serde_json::Value> {
    ...
    match output {
        Ok((stdout, _, _)) => {
            ...
            for line in stdout.lines() {
                ...
                if trimmed.starts_with('│') {
                    ...
                    if parts.len() >= 3 {
                        let name = parts[1].trim().to_string().replace('…', "");
                        let category = parts[2].trim().to_string();
                        if !name.is_empty() && !name.contains("━━━") && !name.starts_with("Name") {
                            skills.push(json!({ "name": name, "category": category }));
                        }
                    }
                }
            }
        ...
        Err(e) => Json(json!({ "skills": [], "error": e })),
    }
}
```

**PITFALL:** Terminal output truncates long names with `…` (e.g. `expo-react-native-mobi…`). Remove the `…` character but accept truncated names — the CLI doesn't expose full names in table mode.
...
- **Discord bot silent or offline**: See `references/discord-setup-troubleshooting.md` for the full troubleshooting guide covering token doubling, config field pitfalls, .env editing restrictions, approval-system blocking, and required Discord Developer Portal settings.
