...
## Pitfall: model API returns 0-byte sizes for LFS files

The Hugging Face **model API** (`/api/models/<repo>`) returns `siblings[].size: 0` for LFS pointer files — those are just pointers, not the actual file content.

**Fix:** Use the **tree API** instead:

```
https://huggingface.co/api/models/<repo>/tree/main?recursive=true
```

The tree API response includes the `size` field with the actual stored file size for every `type: "file"` entry.

### LFS pointer fallback

If the tree API is unreachable or the size still looks unreliable, curl the raw LFS pointer file directly. LFS pointer files are plain text — they contain `version`, `oid`, and `size` fields:

```bash
curl -sL "https://huggingface.co/<repo>/raw/main/<filename>.gguf" | grep "^size " | awk '{print $2}'
```

The pointer file is typically ~1400 bytes, so this is fast even on slow connections. Convert bytes to GB with `numfmt --to=iec` or the standard formula: `bytes / 1073741824`.

**Why this works:** LFS stores files as pointer files in the git repo and actual content in object storage. The raw endpoint serves the pointer file (not the binary) when the client doesn't have LFS, so you always get the metadata as plain text.
