...
- **System font (2026-08-21): EB Garamond** (`aur/ebgaramond-otf`) — replaced Canterbury system-wide (KDE, GTK, terminals, greeter, dconf). Canterbury was fully purged; do not re-suggest it. Medieval Sharp and Junicode were also deployed-then-rejected the same day (Medieval Sharp = same blackletter illegibility class as Canterbury; Junicode lost to EB Garamond on user preference). Do not re-suggest any of the three. Earlier history: synth tried Orbitron as the default on 2026-08-11 and HATED it (especially in Ghostty) — fully reverted the same day. Do not re-suggest Orbitron or proportional/display fonts for terminals. Orbitron files remain at `~/.local/share/fonts/Orbitron/` + Symbols NF at `~/.local/share/fonts/NerdFontsSymbols/` (both unreferenced by any config; fontconfig conf was removed).
- **Font style names are per-family, not universal.** Medieval Sharp's regular face is style `Book`, not `Regular` — Alacritty needs `style = "Book"` in every `[font.*]` block or it resolves oddly. Check real style names with `fc-list | grep -i <name>` before writing terminal configs.
- **Font REPLACEMENT means full purge, not just config edits.** Editing configs while leaving the old font files in `~/.local/share/fonts/<OldFamily>/` means fontconfig still resolves the old family for anything asking by name — looks like the swap "didn't take". Complete replacement: (1) update all config stores (checklist ref below), (2) delete old font files, (3) sweep the repo: `grep -rl "<OldFamily>" <repo> --exclude-dir=.git` — install.sh comments, README tables, skill templates, and GTK SVG assets (`~/.config/gtk-*/assets/*.svg` hardcode `font-family=`) all carry the old name into the next deploy, (4) `fc-cache -f`, (5) restart Plasma (`systemctl --user restart plasma-plasmashell.service`) — panels/taskbar keep the old font until then. User correction 2026-08-21: "stop trying to deploy canterbury just straight up replace it entirely" — half-swaps read as not-done.
...
- Config path: `~/.config/ghostty/config.ghostty`. File must be named `config.ghostty`; Ghostty 1.3+ ignores `config`. (The live dir also has a `config` SYMLINK → config.ghostty, created 2026-08-11 purely for fastfetch's parser — see Ghostty font debugging. Don't delete it, and don't treat it as a second config.)
- **Ghostty is synth's PRIMARY terminal** (his words, 2026-08-11). For any visual change (font, size, palette, padding), verify in Ghostty FIRST — its stderr `font_shared_grid_set` line is ground truth — then mirror to kitty + Alacritty.
...
- **PITFALL (2026-08-21):** the Hermes agent venv's PIL is broken (`ImportError: cannot import name '_imaging'`) and it shadows the system Pillow via PYTHONPATH even when invoking `/usr/bin/python3`. Run specimen/layout scripts as `env -u PYTHONPATH /usr/bin/python3 <script>`.
